...
export class TransactionV2ItemDto {
...
export class GetAllTransactionsV2ResponseDto {
  @ApiProperty({ description: 'List of transactions', type: [TransactionV2ItemDto] })
  items: TransactionV2ItemDto[];

  @ApiProperty({ description: 'Total count' })
  total: number;

  constructor(items: Transaction[], total: number) {
    this.items = items.map((t) => new TransactionV2ItemDto(t));
    this.total = total;
  }
}
