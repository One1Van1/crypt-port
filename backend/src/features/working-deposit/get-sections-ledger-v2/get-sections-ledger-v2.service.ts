...
import {
  ...
  DeficitV2Section,
  FreeUsdtV2Section,
  GetWorkingDepositSectionsLedgerV2ResponseDto,
  ...
  SummaryV2Section,
  WithdrawalV2Dto,
} from './get-sections-ledger-v2.response.dto';

@Injectable()
export class GetWorkingDepositSectionsLedgerV2Service {
  constructor(
  ...
  async execute(): Promise<GetWorkingDepositSectionsLedgerV2ResponseDto> {
    ...
    return new GetWorkingDepositSectionsLedgerV2ResponseDto({
      platformBalances,
      blockedPesos,
      unpaidPesos,
      freeUsdt,
  ...
  private async calculateBlockedPesos(): Promise<PesosAccountsV2Section> {
    ...
    // Keep the section for UI compatibility, but return zeros so charts/totals ignore it.
