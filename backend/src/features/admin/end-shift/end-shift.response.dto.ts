...
export class AdminEndShiftResponseDto {
  ...
  @ApiProperty({ description: 'User id (operator)' })
  userId: number;

  @ApiProperty({ description: 'Username (operator)' })
  username: string;

  @ApiProperty({ description: 'Email (operator)' })
  email: string;
  ...
  constructor(shift: Shift) {
    ...
    this.userId = shift.user?.id;
    this.username = shift.user?.username;
    this.email = shift.user?.email;
  }
}
